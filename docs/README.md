# NopalDB Documentation

## Language Guides

- 🇺🇸 English NQL
- [NQL Reference](en/NQL_REFERENCE.md)
- [NQL Tutorial](en/NQL_TUTORIAL.md)
- 🇲🇽 Español NQL
- [Referencia NQL](es/NQL_REFERENCIA.md)
- [Tutorial NQL](es/NQL_TUTORIAL.md)

## Python Bindings (Bilingual)

- [Python Docs Index](python/README.md)
- [API Reference](python/API_REFERENCE.md)
- [Quick Start](python/QUICKSTART.md)
- [NQL Guide](python/NQL_GUIDE.md)
- [NQL Write CRUD Hands-on](NQL_WRITE_CRUD_HANDS_ON.md)

## Ecosystem

The applications born in this repo now live in their own repositories (AGPL-3.0, with full history):

- [NDBStudio (TUI / web workbench)](https://github.com/Anxious-Mind-Group/ndbstudio)
- [MCP server](https://github.com/Anxious-Mind-Group/nopaldb-mcp)

## Getting Started

- [Feature Tiers & Compilation Guide](FEATURE_TIERS.md) — **Start here** to know how to build by role

## Core Engineering Docs

- [Architecture](ARCHITECTURE.md)
- [SHACL validation](SHACL.md) · [ES](es/SHACL.md)
- [Algorithms](ALGORITHMS.md)
- [Indexing](INDEXING_DOCS.MD)
- [Property Indexing](PROPERTY_INDEXING.md)
- [Query Planner](QUERY_PLANNER_DOCS.MD)
- [Isolation Levels](ISOLATION_LEVELS.md)
- [Deadlock Detection](DEADLOCK_DETECTION.md)

## Arrow & ML Docs

- [Arrow Overview](arrow/01-OVERVIEW.md)
- [Arrow Quickstart](arrow/02-QUICKSTART.md)
- [Arrow Technical](arrow/03-TECHNICAL.md)
- [Arrow Examples](arrow/04-EXAMPLES.md)
- [Arrow ML Integration](arrow/05-ML-INTEGRATION.md)
- [Arrow Performance](arrow/06-PERFORMANCE.md)

## Roadmaps

- [Roadmap](../ROADMAP.md) — direction by theme, and what is explicitly out of scope

## Examples

- [Examples](../nopaldb/examples/)

## Contributing Translations

- [CONTRIBUTING.md](../CONTRIBUTING.md)

## Changelog

- 2026-02-10: NQL docs updated for `EXPORT` to CSV/JSON (with `jsonl`), and documentation indexes cleaned and aligned.
- 2026-02-11: NQL docs standardized to recommended `EXPORT` syntax at query end, using `path="..."` when writing to file.
