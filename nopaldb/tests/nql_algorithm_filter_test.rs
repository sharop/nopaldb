...
async fn setup_graph() -> Result<Graph> {
    ...
    let harbor_cay = tx
        .add_node(
            Node::new("Jurisdiction")
                .with_property("name", PropertyValue::String("Harbor Cay".into())),
        )
        .await?;
    ...
    tx.add_edge(Edge::new(s0, harbor_cay, "REGISTERED_IN"))?;
    tx.add_edge(Edge::new(s1, harbor_cay, "REGISTERED_IN"))?;
    tx.add_edge(Edge::new(s2, harbor_cay, "REGISTERED_IN"))?;
...
async fn test_degree_in_projection_with_pattern_join() -> Result<()> {
    ...
    for row in result.rows() {
        ...
        assert!(
            ...
            "Bug 2: degree(e) returned NULL — conexiones={:?}",
            conexiones
        );
    }
    ...
    assert!(
        matches!(first.get("e.name"), Some(PropertyValue::String(s)) if s == "Acme"),
        "Expected Acme first by degree, got {:?}",
        first.get("e.name")
    );
...
async fn test_degree_in_where_filter() -> Result<()> {
    ...
    assert!(
        matches!(name, Some(PropertyValue::String(s)) if s == "Acme"),
        "Expected Acme, got {:?}",
        name
    );
    Ok(())
...
async fn test_validator_accepts_degree_in_where() -> Result<()> {
    ...
    assert!(
        res.is_ok(),
        "Validator should accept degree() in WHERE. Got: {:?}",
        res.err()
    );
    Ok(())
}
...
async fn test_validator_accepts_degree_in_having() -> Result<()> {
    ...
    assert!(
        res.is_ok(),
        "Validator should accept degree() in HAVING with GROUP BY. Got: {:?}",
        res.err()
    );
    Ok(())
}
...
async fn test_validator_still_rejects_count_in_where() -> Result<()> {
    ...
    assert!(
        msg.contains("Aggregation functions not allowed in WHERE") || msg.contains("aggregation"),
        "Expected aggregation-blocked error, got: {}",
        msg
    );
    Ok(())
}
