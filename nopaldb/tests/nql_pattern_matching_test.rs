...
async fn test_pattern_with_edge_type_filter() -> Result<()> {
    ...
    let acme = Node::new("Company")
        .with_property("name", PropertyValue::String("Acme Corp".into()));
...
async fn test_pattern_projection_all_for_target_and_relationship() -> Result<()> {
    ...
    assert_eq!(row.get("b.name"), Some(&PropertyValue::String("Bob".into())));
    assert_eq!(row.get("b.country"), Some(&PropertyValue::String("MX".into())));
    assert!(row.get("b.id").is_some());
    assert_eq!(row.get("b.label"), Some(&PropertyValue::String("Person".into())));

    assert_eq!(row.get("r.since"), Some(&PropertyValue::Int(2020)));
    assert_eq!(row.get("r.type"), Some(&PropertyValue::String("KNOWS".into())));
...
async fn test_pattern_matching_projection_aliases_return_values() -> Result<()> {
    ...
    let row = &result.rows()[0];
    assert_eq!(row.get("source"), Some(&PropertyValue::String("Medici".into())));
    assert_eq!(row.get("source_faction"), Some(&PropertyValue::String("Bank".into())));
    assert_eq!(row.get("source_wealth"), Some(&PropertyValue::Int(1)));
    assert_eq!(row.get("target"), Some(&PropertyValue::String("Pazzi".into())));
    assert_eq!(row.get("target_faction"), Some(&PropertyValue::String("Trade".into())));
...
async fn test_linear_multihop_pattern_returns_all_bound_nodes() -> Result<()> {
    ...
    assert_eq!(result.len(), 1);
    let row = &result.rows()[0];
    assert_eq!(row.get("a.name"), Some(&PropertyValue::String("Albizzi".into())));
    assert_eq!(row.get("b.name"), Some(&PropertyValue::String("Ridolfi".into())));
    assert_eq!(row.get("c.name"), Some(&PropertyValue::String("Medici".into())));
...
async fn test_linear_multihop_pattern_supports_where_order_by_and_aliases() -> Result<()> {
    ...
    assert_eq!(result.rows()[0].get("bridge"), Some(&PropertyValue::String("Ridolfi".into())));
    assert_eq!(result.rows()[1].get("bridge"), Some(&PropertyValue::String("Salviati".into())));

    for row in result.rows() {
        assert_eq!(row.get("albizzi_family"), Some(&PropertyValue::String("Albizzi".into())));
        assert_eq!(row.get("medici_family"), Some(&PropertyValue::String("Medici".into())));
    }
...
async fn test_multi_pattern_query_joins_shared_variables() -> Result<()> {
    ...
    assert_eq!(result.len(), 2);
    let mut tuples: Vec<(String, String, String)> = result.rows().iter()
        .map(|row| (
            row.get("a").and_then(PropertyValue::as_str).unwrap_or_default().to_string(),
            row.get("b").and_then(PropertyValue::as_str).unwrap_or_default().to_string(),
            row.get("c").and_then(PropertyValue::as_str).unwrap_or_default().to_string(),
        ))
        .collect();
    tuples.sort();

    assert_eq!(tuples, vec![
        ("Albizzi".to_string(), "Medici".to_string(), "Ridolfi".to_string()),
        ("Albizzi".to_string(), "Pazzi".to_string(), "Salviati".to_string()),
    ]);
...
async fn test_distinct_on_multihop_pattern_deduplicates_projected_rows() -> Result<()> {
    ...
    assert_eq!(result.len(), 1);
    let row = &result.rows()[0];
    assert_eq!(row.get("b.name"), Some(&PropertyValue::String("Pazzi".into())));
    assert_eq!(row.get("b.faction"), Some(&PropertyValue::String("Trade".into())));
