use nopaldb::{Graph, Node, Edge, NqlResult, PropertyValue, Result};
...
async fn test_path_metadata_projection_for_fixed_path() -> Result<()> {
    ...
    if let PropertyValue::Object(fields) = &nodes[0] {
        assert_eq!(field(fields, "label"), Some(&PropertyValue::String("Start".to_string())));
        assert!(matches!(field(fields, "id"), Some(PropertyValue::String(_))));
    }
    if let PropertyValue::Object(fields) = &nodes[2] {
        assert_eq!(field(fields, "label"), Some(&PropertyValue::String("End".to_string())));
    }
    ...
    if let PropertyValue::Object(fields) = &edges[0] {
        assert_eq!(field(fields, "type"), Some(&PropertyValue::String("LINK".to_string())));
        assert!(matches!(field(fields, "source"), Some(PropertyValue::String(_))));
        assert!(matches!(field(fields, "target"), Some(PropertyValue::String(_))));
    }
...
async fn test_path_depth_where_and_order_by() -> Result<()> {
    ...
    assert_eq!(result.rows()[0].get("path.depth"), Some(&PropertyValue::Int(2)));
    assert_eq!(result.rows()[0].get("b.name"), Some(&PropertyValue::String("Carol".to_string())));
    assert_eq!(result.rows()[1].get("path.depth"), Some(&PropertyValue::Int(1)));
    assert_eq!(result.rows()[1].get("b.name"), Some(&PropertyValue::String("Bob".to_string())));
...
async fn test_path_nodes_and_edges_rejected_outside_find() -> Result<()> {
    ...
    let where_err = graph.execute_nql(r#"
        find b.id
    ...
    let order_err = graph.execute_nql(r#"
        find b.id
        from (a:Person)-[:KNOWS]->(b:Person)
        order by path.edges
    "#).await.unwrap_err();
...
async fn test_profile_returns_structured_result_for_path_query() -> Result<()> {
    ...
    match result {
        NqlResult::Profile(profile) => {
            ...
            assert_eq!(profile.columns, vec!["b.name".to_string(), "path.depth".to_string()]);
...
async fn test_execute_nql_rejects_profile_statement() -> Result<()> {
    let graph = Graph::in_memory().await?;
    let err = graph.execute_nql("profile find n.id from (n:Person)").await.unwrap_err();
...
fn test_property_value_messagepack_roundtrip_for_structured_values() {
        ...
        (
            ...
            PropertyValue::List(vec![
                PropertyValue::Object(vec![
                    ...
                    ("label".to_string(), PropertyValue::String("Start".to_string())),
                ]),
                PropertyValue::Object(vec![
                    ("id".to_string(), PropertyValue::String("n2".to_string())),
                    ("label".to_string(), PropertyValue::String("End".to_string())),
                ]),
            ]),
        ),
    ]);
    ...
    let decoded: PropertyValue = rmp_serde::from_slice(&bytes).expect("MessagePack deserialization must work");
