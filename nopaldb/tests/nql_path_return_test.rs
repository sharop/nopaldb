...
fn bool_val(v: bool) -> PropertyValue { PropertyValue::Bool(v) }
fn str_val(s: &str) -> PropertyValue { PropertyValue::String(s.to_string()) }
...
fn get_object_field(val: &PropertyValue, field: &str) -> Option<PropertyValue> {
    if let PropertyValue::Object(entries) = val {
        entries.iter().find(|(k, _)| k == field).map(|(_, v)| v.clone())
    } else {
        None
    }
}
...
async fn test_path_result_in_find() -> Result<()> {
    ...
    let a = tx.add_node(Node::new("Account").with_property("name", str_val("A"))).await?;
    let b = tx.add_node(Node::new("Account").with_property("name", str_val("B"))).await?;
    let c = tx.add_node(Node::new("Account").with_property("name", str_val("C"))).await?;
    ...
    let mut scores: Vec<i64> = result.rows().iter()
        ...
        .filter_map(|v| if let PropertyValue::Int(n) = v { Some(*n) } else { None })
        .collect();
    scores.sort_unstable();

    assert!(scores.contains(&100), "score 100 (A->B) expected, got {:?}", scores);
    assert!(scores.contains(&150), "score 150 (A->B->C) expected, got {:?}", scores);
...
async fn test_path_start_end_in_find() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let a = tx.add_node(Node::new("Wallet").with_property("name", str_val("Alice"))).await?;
    let b = tx.add_node(Node::new("Wallet").with_property("name", str_val("Bob"))).await?;
...
async fn test_return_evaluated_once_per_path() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let hub  = tx.add_node(Node::new("Hub").with_property("name", str_val("HUB"))).await?;
    let p1   = tx.add_node(Node::new("Peer").with_property("name", str_val("P1"))).await?;
    let p2   = tx.add_node(Node::new("Peer").with_property("name", str_val("P2"))).await?;
    ...
    let mut weights: Vec<i64> = result.rows().iter()
        .filter_map(|r| r.get("w"))
        .filter_map(|v| if let PropertyValue::Int(n) = v { Some(*n) } else { None })
        .collect();
    weights.sort_unstable();
    assert_eq!(weights, vec![10, 30], "each path should have its own result");
...
async fn test_return_boolean_result() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let a = tx.add_node(Node::new("Node").with_property("name", str_val("A"))).await?;
    let b = tx.add_node(Node::new("Node").with_property("name", str_val("B"))).await?;
    let c = tx.add_node(Node::new("Node").with_property("name", str_val("C"))).await?;
...
async fn test_return_numeric_with_depth() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let a = tx.add_node(Node::new("N").with_property("name", str_val("A"))).await?;
    let b = tx.add_node(Node::new("N").with_property("name", str_val("B"))).await?;
...
async fn test_path_result_without_return_fails() -> Result<()> {
    ...
    assert!(msg.contains("path.result requires a RETURN"), "unexpected: {}", msg);
...
async fn test_path_state_in_where_fails() -> Result<()> {
    ...
    assert!(err.is_err());
    let msg = err.unwrap_err().to_string();
    assert!(msg.contains("path.state is not allowed in WHERE"), "unexpected: {}", msg);
...
async fn test_path_start_in_where_fails() -> Result<()> {
    ...
    assert!(err.is_err());
    let msg = err.unwrap_err().to_string();
    assert!(msg.contains("path.start is not allowed in WHERE"), "unexpected: {}", msg);
...
async fn test_return_object_fails() -> Result<()> {
    ...
    assert!(err.is_err());
    let msg = err.unwrap_err().to_string();
    assert!(msg.contains("scalar"), "expected scalar error, got: {}", msg);
...
async fn test_return_with_order_by_fails() -> Result<()> {
    ...
    assert!(err.is_err());
    let msg = err.unwrap_err().to_string();
    assert!(msg.contains("RETURN is not supported with ORDER BY"), "unexpected: {}", msg);
