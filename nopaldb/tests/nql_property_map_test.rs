...
async fn setup() -> (Graph, [uuid::Uuid; 5]) {
    ...
    // WORKS_AT
    graph.add_edge(Edge::new(alice.id, acme.id, "WORKS_AT")).await.unwrap();
    graph.add_edge(Edge::new(bob.id, beta.id, "WORKS_AT")).await.unwrap();
    graph.add_edge(Edge::new(carol.id, acme.id, "WORKS_AT")).await.unwrap();

    // KNOWS
    graph.add_edge(Edge::new(alice.id, bob.id, "KNOWS")).await.unwrap();
    graph.add_edge(Edge::new(alice.id, carol.id, "KNOWS")).await.unwrap();
...
fn names(result: &nopaldb::query::nql::executor::result::QueryResult, col: &str) -> Vec<String> {
    let mut v: Vec<_> = result.rows.iter()
...
async fn test_source_property_map_single_field() {
    ...
    let r = graph.execute_nql(
        r#"FIND a.name FROM (a:Person {dept: "eng"}) -> [:WORKS_AT] -> (b:Company)"#
    ).await.unwrap();
...
async fn test_source_property_map_bool_field() {
    ...
    let r = graph.execute_nql(
        "FIND a.name FROM (a:Person {active: true}) -> [:WORKS_AT] -> (b:Company)"
    ).await.unwrap();
    assert_eq!(names(&r, "a.name"), vec!["Alice", "Carol"]);
}
...
async fn test_source_property_map_no_match_returns_empty() {
    ...
    assert!(r.rows.is_empty(), "no match expected, got {:?}", r.rows.len());
}
...
async fn test_target_property_map_bool_field() {
    ...
    let r = graph.execute_nql(
        "FIND a.name FROM (a:Person) -> [:WORKS_AT] -> (b:Company {public: false})"
    ).await.unwrap();
...
async fn test_source_and_target_property_maps() {
    ...
    for row in &r.rows {
        assert_eq!(row.get("b.name"), Some(&PropertyValue::String("Acme".into())));
    }
}
...
async fn test_single_node_property_map_regression() {
    let (graph, _) = setup().await;
    let r = graph.execute_nql(
        r#"FIND n.name FROM (n:Person {dept: "eng"})"#
    ).await.unwrap();
...
async fn test_property_map_knows_relationship() {
    ...
    assert_eq!(r.rows[0].get("b.name"), Some(&PropertyValue::String("Carol".into())));
}
