...
async fn test_streaming_executor_scan_with_label() -> anyhow::Result<()> {
    ...
    graph.execute_nql(r#"add (n:Person {name: "Alice", age: 30})"#).await?;
    graph.execute_nql(r#"add (n:Person {name: "Bob", age: 25})"#).await?;
    ...
    let result = graph.execute_nql(r#"
        ...
        from (n:Person)
    "#).await?;
    assert_eq!(result.rows.len(), 2, "Expected 2 Person nodes, got {}", result.rows.len());
...
async fn test_streaming_executor_filter() -> anyhow::Result<()> {
    ...
    let result = graph.execute_nql(r#"
        ...
        where n.age > 28
    "#).await?;
    assert_eq!(result.rows.len(), 1, "Expected 1 node where age > 28, got {}", result.rows.len());
...
async fn test_streaming_executor_scan_all() -> anyhow::Result<()> {
    ...
    graph.execute_nql(r#"add (n:Person {name: "Alice"})"#).await?;
    graph.execute_nql(r#"add (n:Robot {name: "R2D2"})"#).await?;
    ...
    let result = graph.execute_nql(r#"
        ...
        from (n)
    "#).await?;
    assert_eq!(result.rows.len(), 2, "Expected all 2 nodes, got {}", result.rows.len());
...
async fn test_streaming_executor_order_by_hidden_column() -> anyhow::Result<()> {
    ...
    graph.execute_nql(r#"add (n:Person {name: "Alice", age: 30})"#).await?;
    graph.execute_nql(r#"add (n:Person {name: "Bob", age: 25})"#).await?;
    graph.execute_nql(r#"add (n:Person {name: "Charlie", age: 35})"#).await?;
    ...
    assert_eq!(result.rows[0].get("n.name"), Some(&PropertyValue::String("Charlie".to_string())));
    assert_eq!(result.rows[1].get("n.name"), Some(&PropertyValue::String("Alice".to_string())));
    assert_eq!(result.rows[2].get("n.name"), Some(&PropertyValue::String(" Bob".trim().to_string()))); // trimming just in case

    // Verify n.age IS NOT in the final rows (it should have been stripped)
    assert!(result.rows[0].get("n.age").is_none(), "Extra column n.age should have been stripped");
...
async fn test_streaming_executor_relationship_match() -> anyhow::Result<()> {
    ...
    let alice_id = graph.add_node(Node::new("Person").with_properties(alice_props)).await?;
    ...
    let bob_id = graph.add_node(Node::new("Person").with_properties(bob_props)).await?;

    let mut edge_props = HashMap::new();
    edge_props.insert("since".to_string(), PropertyValue::Int(2020));
    graph.add_edge(
        Edge::new(alice_id, bob_id, "KNOWS").with_properties(edge_props)
    ).await?;
    ...
    assert_eq!(result.rows[0].get("a.name").unwrap().as_str().unwrap(), "Alice");
