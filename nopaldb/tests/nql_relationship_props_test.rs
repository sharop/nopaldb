...
fn col_sorted(r: &nopaldb::query::nql::executor::result::QueryResult, col: &str) -> Vec<String> {
    let mut v: Vec<_> = r.rows.iter().filter_map(|row| {
        if let Some(PropertyValue::String(s)) = row.get(col) { Some(s.clone()) } else { None }
    }).collect();
    v.sort();
    v
}
...
async fn test_rel_props_single_int_filter() {
    ...
    assert_eq!(r.rows.len(), 2, "Esperaba 2 transferencias con amount=5000, got {}", r.rows.len());
}
...
async fn test_rel_props_no_match() {
    ...
    assert!(r.rows.is_empty(), "Esperaba 0 resultados, got {}", r.rows.len());
}
...
async fn test_rel_props_multi_filter() {
    ...
    assert_eq!(r.rows.len(), 1, "Esperaba 1 resultado, got {}", r.rows.len());
    assert_eq!(r.rows[0].get("a.name"), Some(&PropertyValue::String("Alice".into())));
    assert_eq!(r.rows[0].get("b.name"), Some(&PropertyValue::String("Bob".into())));
}
...
async fn test_rel_props_access_edge_var_with_inline_filter() {
    ...
    assert_eq!(r.rows.len(), 2, "Esperaba 2 transferencias en USD, got {}", r.rows.len());

    let mut amounts: Vec<i64> = r.rows.iter()
        .filter_map(|row| {
            if let Some(PropertyValue::Int(v)) = row.get("r.amount") { Some(*v) } else { None }
        })
        .collect();
    amounts.sort();
