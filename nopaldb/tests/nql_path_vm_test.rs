...
async fn test_path_vm_boolean_assignment_from_comparison() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let a = tx
        .add_node(Node::new("Company").with_property("name", PropertyValue::String("A".into())))
        .await?;
    let b = tx
        .add_node(Node::new("Company").with_property("name", PropertyValue::String("B".into())))
        .await?;
    let c = tx
        .add_node(Node::new("Company").with_property("name", PropertyValue::String("C".into())))
        .await?;
    ...
    assert_eq!(result.len(), 2);
    assert_eq!(
        result.rows()[0].get("risky"),
        Some(&PropertyValue::Bool(false))
    );
    assert_eq!(
        result.rows()[1].get("risky"),
...
async fn test_path_vm_boolean_and_where_filter() -> Result<()> {
    ...
    tx.add_edge(
        Edge::new(a, b, "OWNS")
            .with_property("risk_score", int(20))
            .with_property("amount", int(50)),
    )?;
    tx.add_edge(
        Edge::new(b, c, "OWNS")
            .with_property("risk_score", int(90))
            .with_property("amount", int(120)),
    )?;
    tx.commit().await?;
