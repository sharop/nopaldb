...
async fn test_p0a_add_simple_node_transactional() -> Result<()> {
    ...
    graph.execute_statement(r#"add (x:Test {val: 42})"#).await?;

    let nodes = graph.get_nodes_by_label("Test").await?;
    assert_eq!(nodes.len(), 1);
    assert_eq!(
        nodes[0].properties.get("val"),
...
async fn test_p0b_delete_with_relationship_pattern() -> Result<()> {
    ...
    let charlie = tx
        ...
        .await?;
    let _dave = tx
        .add_node(Node::new("Person").with_property("name", PropertyValue::String("Dave".into())))
        .await?;
    tx.commit().await?;
    ...
    let result = graph
        .execute_statement(r#"delete (a:Person)-[:KNOWS]->(b:Person) where b.name = "Bob""#)
        .await?;
    ...
    match &result {
        NqlResult::Write(w) => {
            ...
            assert_eq!(
                w.nodes_deleted, 0,
                "Relationship delete should preserve matched nodes"
            );
        }
        other => panic!("Expected Write, got {:?}", other),
    }
    ...
    assert_eq!(
        edges.len(),
        1,
        "Only the matching relationship should be deleted"
    );
...
async fn test_p0b_update_with_relationship_pattern() -> Result<()> {
    ...
    let result = graph
        .execute_statement(r#"update (a:Person)-[:MANAGES]->(b:Person) set b.role = "senior""#)
        .await?;
    ...
    for node in &nodes {
        if let Some(PropertyValue::String(name)) = node.properties.get("name") {
            ...
            if name == "Alice" {
                assert_eq!(
                    role,
                    Some(&PropertyValue::String("manager".into())),
                    "Alice should still be manager"
                );
            } else {
                assert_eq!(
                    role,
                    Some(&PropertyValue::String("senior".into())),
                    "{} should be senior now",
                    name
                );
            }
        }
