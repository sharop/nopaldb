...
async fn test_exact_depth_2() -> Result<()> {
    ...
    let diana   = tx.add_node(Node::new("Person").with_property("name", PropertyValue::String("Diana".into()))).await?;
    ...
    let names: Vec<_> = result.rows().iter()
        .filter_map(|r| r.get("b.name"))
        .collect();

    assert_eq!(names.len(), 1, "Expected exactly 1 result at depth 2, got: {:?}", names);
    assert!(
        matches!(names[0], PropertyValue::String(s) if s == "Charlie"),
        "Expected Charlie at depth 2, got: {:?}", names[0]
    );
...
async fn test_range_depth_1_to_3() -> Result<()> {
    ...
    assert_eq!(names.len(), 3, "Expected Bob, Charlie, Diana — got: {:?}", names);
...
async fn test_cycle_safety() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let alfa = tx.add_node(Node::new("Person").with_property("name", PropertyValue::String("Alfa".into()))).await?;
    let beta = tx.add_node(Node::new("Person").with_property("name", PropertyValue::String("Beta".into()))).await?;
...
async fn test_edge_property_filter_quantified() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let a1 = tx.add_node(Node::new("Account").with_property("name", PropertyValue::String("A1".into()))).await?;
    let a2 = tx.add_node(Node::new("Account").with_property("name", PropertyValue::String("A2".into()))).await?;
    let a3 = tx.add_node(Node::new("Account").with_property("name", PropertyValue::String("A3".into()))).await?;
    // Primera arista: risk = "high"
    tx.add_edge(Edge::new(a1, a2, "TRANSFER").with_property("risk", PropertyValue::String("high".into())))?;
    // Segunda arista: risk = "low" (debe ser filtrada)
    tx.add_edge(Edge::new(a2, a3, "TRANSFER").with_property("risk", PropertyValue::String("low".into())))?;
    tx.commit().await?;
    ...
    assert!(names.iter().any(|n| n == "A2"), "Expected A2 via high-risk edge, got: {:?}", names);
    assert!(!names.iter().any(|n| n == "A3"), "A3 must NOT appear (edge has low risk), got: {:?}", names);
...
async fn test_terminal_pattern_only_on_final() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let s = tx.add_node(Node::new("Start").with_property("name", PropertyValue::String("S".into()))).await?;
    let m = tx.add_node(Node::new("Middle").with_property("name", PropertyValue::String("M".into()))).await?;
    let e = tx.add_node(Node::new("End").with_property("name", PropertyValue::String("E".into()))).await?;
    ...
    assert_eq!(names.len(), 1, "Expected End node at depth 2, got: {:?}", names);
    assert_eq!(names[0], "E");
...
async fn test_unbounded_quantifier_rejected() -> Result<()> {
    ...
    let mut tx = graph.begin_transaction().await?;
    let a = tx.add_node(Node::new("Person").with_property("name", PropertyValue::String("A".into()))).await?;
    let b = tx.add_node(Node::new("Person").with_property("name", PropertyValue::String("B".into()))).await?;
    ...
    assert!(
        ...
        "Error must mention F1/upper bound, got: {}", err
    );
...
async fn test_quantified_relationship_variable_rejected() -> Result<()> {
    ...
    assert!(result.is_err(), "Quantified relationship variable must return an error");
    let err = format!("{}", result.unwrap_err());
    assert!(
        err.contains("variable") || err.contains("F1"),
        "Error must mention variable/F1, got: {}", err
    );
...
async fn test_mixed_fixed_and_quantified_hops() -> Result<()> {
    ...
    assert!(names.iter().any(|n| n == "Bob"), "Expected Bob in mixed pattern, got: {:?}", names);
    assert!(names.iter().any(|n| n == "Charlie"), "Expected Charlie in mixed pattern, got: {:?}", names);
    assert!(!names.iter().any(|n| n == "Alice"), "Alice is anchor, not result, got: {:?}", names);
...
async fn test_zero_min_quantifier_rejected() -> Result<()> {
    ...
    assert!(
        ...
        "Error must mention zero-hop/min/F1, got: {}", err1
    );

    // {0,3} — range starting at zero
    let r2 = graph.execute_nql(r#"
        find b.name
    ...
    assert!(
        ...
        "Error must mention zero-hop/min/F1, got: {}", err2
    );
