...
fn test_update_multiple_assignments() {
    ...
    assert!(result.is_ok(), "Failed to parse UPDATE with multiple assignments: {:?}", result.err());
...
fn test_update_with_limit() {
    ...
    assert!(result.is_ok(), "Failed to parse UPDATE with LIMIT: {:?}", result.err());
...
fn test_update_without_where() {
    ...
    assert!(result.is_ok(), "Failed to parse UPDATE without WHERE: {:?}", result.err());
    
    if let Statement::Update(upd) = result.unwrap() {
        assert!(upd.filter.is_none());
        assert_eq!(upd.danger_level(), nopaldb::query::nql::parser::ast::DangerLevel::High);
    }
}
