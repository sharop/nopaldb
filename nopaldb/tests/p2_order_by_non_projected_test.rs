...
async fn test_order_by_non_projected_column() -> Result<()> {
    ...
    assert_eq!(names, vec!["Alice", "Diana", "Bob", "Charlie"],
               "Should be ordered by age ascending, got {:?}", names);

    // Verify age column is NOT in the result (was stripped)
    assert!(!result.columns.contains(&"p.age".to_string()),
            "p.age should not be in result columns: {:?}", result.columns);
...
async fn test_order_by_non_projected_desc() -> Result<()> {
    ...
    assert_eq!(names, vec!["B", "C", "D", "A"],
               "Should be ordered by score descending, got {:?}", names);
...
async fn test_order_by_non_projected_with_limit() -> Result<()> {
    ...
    assert_eq!(names, vec!["item_9", "item_8", "item_7"],
               "Should get top 3 items by rank desc, got {:?}", names);
