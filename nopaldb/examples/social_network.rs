...
async fn main() {
    ...
    graph
        .add_edge(
            Edge::new(bob_id, charlie_id, "KNOWS").with_property("since", PropertyValue::Int(2019)),
        )
        .await
        .unwrap();

    graph
        .add_edge(Edge::new(alice_id, charlie_id, "WORKS_WITH"))
        .await
        .unwrap();
    ...
    println!(
        "\nGrado de Bob: {}",
        graph.degree(bob_id, Direction::Both).await.unwrap()
    );
}
