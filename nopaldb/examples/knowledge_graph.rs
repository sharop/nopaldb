...
async fn main() -> nopaldb::Result<()> {
    ...
    let domains = graph.traverse(rust)
        .out_e("USED_IN")
        .has_label("Domain")
        .nodes()
        .await?;
    ...
    let concepts = graph.traverse(rust)
        .out_e("PROVIDES")
        .nodes()
        .await?;
    ...
    let web_langs = graph.traverse(web_dev)
        .in_e("USED_IN")
        .has_label("Language")
        .nodes()
        .await?;
