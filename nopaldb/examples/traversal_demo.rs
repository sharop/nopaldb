use nopaldb::{Direction, Edge, Graph, Node, TraversalConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    graph
        .add_edge(Edge::new(node1, node2, "KNOWS"))
        .await
        .unwrap();
    graph
        .add_edge(Edge::new(node1, node3, "KNOWS"))
        .await
        .unwrap();
    graph
        .add_edge(Edge::new(node2, node4, "KNOWS"))
        .await
        .unwrap();
    graph
        .add_edge(Edge::new(node2, node5, "KNOWS"))
        .await
        .unwrap();
    graph
        .add_edge(Edge::new(node3, node6, "KNOWS"))
        .await
        .unwrap();
        ...
        .await?
    {
        Some(path_result) => {
            println!(
                "Camino encontrado ({} saltos):",
                path_result.nodes.len() - 1
            );
