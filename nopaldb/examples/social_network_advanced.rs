...
async fn main() -> nopaldb::Result<()>{
    ...
    let diana = graph.add_node(Node::new("Person")
    ...
    let eve = graph.add_node(Node::new("Person")
    ...
    // === Crear lugares ===
    let cdmx = graph.add_node(Node::new("City")
        .with_property("name", PropertyValue::String("CDMX".into()))
    ).await?;
    
    let gdl = graph.add_node(Node::new("City")
        .with_property("name", PropertyValue::String("Guadalajara".into()))
    ).await?;
    
    // === Relaciones sociales ===
    graph.add_edge(Edge::new(alice, bob, "KNOWS")
        .with_property("since", PropertyValue::Int(2020))
    ).await?;
    
    graph.add_edge(Edge::new(alice, charlie, "KNOWS")
        .with_property("since", PropertyValue::Int(2019))
    ).await?;
    
    graph.add_edge(Edge::new(bob, diana, "KNOWS")
        .with_property("since", PropertyValue::Int(2021))
    ).await?;
    
    graph.add_edge(Edge::new(charlie, eve, "KNOWS")
        .with_property("since", PropertyValue::Int(2018))
    ).await?;
    
    graph.add_edge(Edge::new(alice, diana, "WORKS_WITH")).await?;
    ...
    let older_friends = graph.traverse(alice)
        .out_e("KNOWS")
        .filter(|node| {
            matches!(
    ...
    let rust_lovers = graph.traverse(alice)
        .out_e("KNOWS")
        .out_e("KNOWS")
        .has_label("Person")
        .filter(|node| {
    ...
    let limited = graph.traverse(alice)
        .out()
        .limit(2)
        .nodes()
        .await?;
