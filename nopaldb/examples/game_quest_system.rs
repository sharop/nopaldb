...
async fn main() -> nopaldb::Result<()> {
    ...
    let main_quest = graph.add_node(Node::new("Quest")
    ...
    let gather_herbs = graph.add_node(Node::new("Quest")
    ...
    let defeat_bandits = graph.add_node(Node::new("Quest")
    ...
    graph.add_edge(Edge::new(gather_herbs, main_quest, "UNLOCKS")
        .with_property("progress_required", PropertyValue::Int(100))
    ).await?;

    graph.add_edge(Edge::new(defeat_bandits, main_quest, "UNLOCKS")
        .with_property("progress_required", PropertyValue::Int(100))
    ).await?;

    // Chain: Gathering herbs unlocks bandit quest
    graph.add_edge(Edge::new(gather_herbs, defeat_bandits, "UNLOCKS")).await?;

    println!("🎮 Quest System Initialized!");
    println!("\n📜 Available Quests:");

    // Find available quests
    let available = graph.find_nodes_by_property(
        "status",
        &PropertyValue::String("available".into())
    ).await?;
    ...
    println!("\n🔒 Locked Quests:");
    let locked = graph.find_nodes_by_property(
        "status",
        &PropertyValue::String("locked".into())
    ).await?;
