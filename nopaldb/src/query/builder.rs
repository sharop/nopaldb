...
use crate::query::step::{TraversalStep, TraversalState};
...
impl TraverseBuilder {
    ...
    pub fn has_label(mut self, label: impl Into<String>) -> Self {
        ...
        self.predicates.push(Arc::new(move |node: &Node| node.label == label_clone));
        self.steps.push(TraversalStep::Filter {
        ...
        for step in &self.steps {
            match step {
                TraversalStep::FollowEdge { edge_type, direction } => {
                    current_nodes = self.follow_edges(
                        current_nodes,
                        edge_type.as_deref(),
                        *direction,
                    ).await?;
                ...
                TraversalStep::Filter { .. } => {
                    if predicate_idx < self.predicates.len() {
                        current_nodes = self.apply_filter(
                            current_nodes,
                            &self.predicates[predicate_idx],
                        ).await?;
                        predicate_idx += 1;
                    }
                }
...
mod tests {
    ...
    async fn create_social_graph() -> (Graph, NodeId) {
        ...
        let david = graph.add_node(Node::new("Person")
        ...
        graph.add_edge(Edge::new(alice, bob, "KNOWS")).await.unwrap();
        graph.add_edge(Edge::new(alice, david, "KNOWS")).await.unwrap();
        graph.add_edge(Edge::new(bob, charlie, "KNOWS")).await.unwrap();
    ...
    async fn test_simple_traverse() {
        ...
        let result = graph.traverse(alice_id)
            .out()
            .execute()
            .await
            .unwrap();
